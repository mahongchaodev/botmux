...
3. 执行 `pnpm link --global`，让全局 `botmux` 指向这份源码。
4. 使用 electron-builder 生成本机 `Botmux.app`。
...
`--skip-build` 适合已经运行过 `pnpm build && pnpm desktop:bundle && pnpm exec electron-builder --mac dir --config electron-builder.yml` 的开发场景。
...
因为 App 始终依赖全局 `botmux` CLI，脚本会重新 `pnpm link --global`，保证 App 和 CLI 指向同一份源码。
