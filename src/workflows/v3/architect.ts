...
export function buildArchitectGoal(specPath: string, specJsonPath: string): string {
  ...
  return [
    ...
    '- A goal node (incl. loop body nodes) may set `override: { model?, permissionMode?, systemPromptAppend? }` — restrict or redirect only, never escalate.',
    ...
    '- `permissionMode: "restricted"`: use for read-only analysis/review nodes that must not modify files or run privileged commands. There is NO "bypass" value — a node can never gain more permission than its bot.',
